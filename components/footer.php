    <?php include_once __DIR__ . '/calltoaction.php'; ?>
  </main>

  <footer class="bg-white pt-16 pb-8 px-6 lg:px-16 border-t-4 border-secondary shadow-inner">
      <div class="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start gap-12">
        <div class="flex-1 flex flex-col">
          <h2 class="text-3xl font-bold text-primary mb-5">Display Doctor</h2>

          <address class="not-italic flex flex-col gap-4 mb-6 text-gray-600">
            <a href="mailto:displaydoctorykolkata@gmail.com"
              class="flex items-center gap-3 hover:text-blue-600 transition-colors duration-300 font-medium">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                stroke="currentColor" class="size-5 shrink-0 text-secondary">
                <path stroke-linecap="round" stroke-linejoin="round"
                  d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75" />
              </svg>
              info@displaydoctor.in
            </a>

            <a href="tel:+917605884658"
              class="flex items-center gap-3 text-lg font-semibold hover:text-blue-600 transition-colors duration-300">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                stroke="currentColor" class="size-5 shrink-0 text-secondary">
                <path stroke-linecap="round" stroke-linejoin="round"
                  d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 0 0 2.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 0 1-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 0 0-1.091-.852H4.5A2.25 2.25 0 0 0 2.25 4.5v2.25Z" />
              </svg>
              +91 7605884658
            </a>
          </address>

          <div class="flex items-center gap-5 text-gray-500">
            <a href="#" target="_blank" rel="noopener noreferrer" aria-label="Visit our Facebook page"
              class="hover:text-blue-600 transition-colors duration-300 hover:scale-110 transform">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-6">
                <path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3.6l.4-4h-4V7a1 1 0 0 1 1-1h3z"></path>
              </svg>
            </a>

            <a href="#" target="_blank" rel="noopener noreferrer" aria-label="Visit our Instagram page"
              class="hover:text-pink-600 transition-colors duration-300 hover:scale-110 transform">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-6">
                <rect x="2" y="2" width="20" height="20" rx="5" ry="5"></rect>
                <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path>
                <line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line>
              </svg>
            </a>

            <a href="#" target="_blank" rel="noopener noreferrer" aria-label="Visit our YouTube channel"
              class="hover:text-red-600 transition-colors duration-300 hover:scale-110 transform">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-6">
                <path
                  d="M22.54 6.42a2.78 2.78 0 0 0-1.94-2C18.88 4 12 4 12 4s-6.88 0-8.6.46a2.78 2.78 0 0 0-1.94 2A29 29 0 0 0 1 11.75a29 29 0 0 0 .46 5.33A2.78 2.78 0 0 0 3.4 19c1.72.46 8.6.46 8.6.46s6.88 0 8.6-.46a2.78 2.78 0 0 0 1.94-2 29 29 0 0 0 .46-5.25 29 29 0 0 0-.46-5.33z">
                </path>
                <polygon points="9.75 15.02 15.5 11.75 9.75 8.48 9.75 15.02"></polygon>
              </svg>
            </a>
          </div>
        </div>

        <div class="flex-1 w-full md:flex md:flex-col md:items-end">
          <h3 class="text-xl font-bold text-primary mb-5">Quick Links</h3>
          <ul class="flex flex-col gap-3 md:items-end text-gray-600 font-medium">
            <li>
              <a href="/index.php"
                class="hover:text-secondary hover:underline underline-offset-4 transition-all duration-300">Home</a>
            </li>
            <li>
              <a href="#"
                class="hover:text-secondary hover:underline underline-offset-4 transition-all duration-300">Our
                Services</a>
            </li>
            <li>
              <a href="#"
                class="hover:text-secondary hover:underline underline-offset-4 transition-all duration-300">Privacy
                Policy</a>
            </li>
            <li>
              <a href="#"
                class="hover:text-secondary hover:underline underline-offset-4 transition-all duration-300">Terms &
                Conditions</a>
            </li>
          </ul>
        </div>
      </div>

      <div
        class="max-w-7xl mx-auto mt-12 pt-6 border-t border-gray-200 text-center md:text-left flex flex-col md:flex-row justify-between items-center gap-4 text-sm text-gray-500">
        <p>&copy; <?php echo date("Y"); ?> Display Doctor. All rights reserved.</p>
        <p>Expert TV Repair in Kolkata</p>
      </div>
    </footer>

  <script src="/js/main.js"></script>
</body>
</html>
